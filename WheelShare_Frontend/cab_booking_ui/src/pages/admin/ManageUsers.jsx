...
function ManageUsers() {
  ...
  const handleViewProfile = async (user) => {
    try {
      const response = await api.get(`/admin/users/${user.userId}/profile`);
  ...
  const handleRejectUser = async (userId) => {
    try {
      ...
      setProfileData(prev => ({ ...prev, isVerified: false }));
      fetchUsers();
                  ...
                  <td style={{ padding: '0.75rem 0.5rem', fontWeight: '500' }}>
                    {user.role === 'DRIVER' ? (
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                          console.log('Clicking driver:', user);
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h3 style={{ margin: 0 }}>Driver Profile - {profileData.name}</h3>
              <button 
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              <div className="card" style={{ padding: '1rem' }}>
                <h4 style={{ margin: '0 0 1rem 0', color: '#007bff' }}>Basic Information</h4>
                <div style={{ display: 'grid', gap: '0.5rem' }}>
                  ...
                  <div><strong>Verified:</strong> {profileData.isVerified ? 'Yes' : 'No'}</div>
                </div>
              </div>
              
              {profileData.driverProfile && (
                <div className="card" style={{ padding: '1rem' }}>
                  <h4 style={{ margin: '0 0 1rem 0', color: '#28a745' }}>Driver Details</h4>
                  <div style={{ display: 'grid', gap: '0.5rem' }}>
                    <div><strong>License Number:</strong> {profileData.driverProfile.licenseNumber || 'Not provided'}</div>
                    <div><strong>Vehicle Model:</strong> {profileData.driverProfile.vehicleModel || 'Not provided'}</div>
                    <div><strong>Registration Number:</strong> {profileData.driverProfile.registrationNumber || 'Not provided'}</div>
                    <div><strong>Currently Available:</strong> {profileData.driverProfile.isAvailable ? 'Yes' : 'No'}</div>
                    {profileData.driverProfile.currentLatitude && profileData.driverProfile.currentLongitude && (
                      <div><strong>Last Location:</strong> {profileData.driverProfile.currentLatitude.toFixed(6)}, {profileData.driverProfile.currentLongitude.toFixed(6)}</div>
                    )}
                  </div>
                </div>
              )}
            ...
            {profileData.role === 'DRIVER' && (
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', marginTop: '1.5rem' }}>
                <button
                  onClick={() => handleVerifyUser(profileData.userId)}
                ...
                <button
                  onClick={() => handleRejectUser(profileData.userId)}
