...
import { MapPin, Navigation, Clock, Car, Phone, CheckCircle, AlertCircle, Loader, XCircle, CreditCard, Banknote, Receipt as ReceiptIcon } from "lucide-react";
...
import { getPendingRequests, approveRequest, rejectRequest, confirmSeat, completeRide } from "../services/carpoolService";
import MapplsMap from "../components/MapplsMap";
import Receipt from "../components/Receipt";
import CarpoolApprovalModal from "../components/CarpoolApprovalModal";
import SeatConfirmationModal from "../components/SeatConfirmationModal";
...
function RideStatus() {
  ...
  const [carpoolRequest, setCarpoolRequest] = useState(null);
  ...
  useEffect(() => {
    ...
    const fetchRideStatus = async () => {
      try {
        ...
        if (rideData?.status !== "COMPLETED" && newRideData.status === "COMPLETED") {
          ...
          if (!newRideData.paymentStatus || newRideData.paymentStatus !== 'COMPLETED') {
    ...
    const fetchCarpoolRequests = async () => {
      try {
        const response = await getPendingRequests(rideId);
        if (response.data && response.data.length > 0) {
          setCarpoolRequest(response.data[0]);
        } else {
          setCarpoolRequest(null);
        }
      } catch (err) {
        console.error("Error fetching carpool requests:", err);
      }
    };
    ...
    const interval = setInterval(() => {
      fetchRideStatus();
      if (rideData?.isCarpool && !rideData?.parentRideId) {
        fetchCarpoolRequests();
      }
    }, 2000);
  ...
  // Check if ride was approved and show seat confirmation for carpool passengers
  useEffect(() => {
    if (rideData?.isCarpool && rideData?.parentRideId && 
        (rideData.status === "ACCEPTED" || rideData.status === "STARTED") && 
        !seatConfirmed && !showSeatConfirmation) {
      setShowSeatConfirmation(true);
    }
  }, [rideData?.status, rideData?.isCarpool, rideData?.parentRideId, seatConfirmed, showSeatConfirmation]);

  // Check if carpool passenger should complete ride
  useEffect(() => {
    if (rideData?.isCarpool && rideData?.parentRideId && 
        rideData.status === "STARTED" && seatConfirmed && 
        !showRideCompletion && !paymentCompleted) {
      // Show completion modal after some time in ride
      const timer = setTimeout(() => {
        setShowRideCompletion(true);
      }, 30000); // Show after 30 seconds for demo, in real app this would be based on location
      
      return () => clearTimeout(timer);
    }
  }, [rideData?.status, rideData?.isCarpool, rideData?.parentRideId, seatConfirmed, showRideCompletion, paymentCompleted]);

  const handleSeatConfirm = async () => {
    try {
      await confirmSeat(rideId);
      setSeatConfirmed(true);
      setShowSeatConfirmation(false);
      success("Seat confirmed! Enjoy your ride.");
    } catch (err) {
      showError("Failed to confirm seat");
    }
  };

  const handleSeatDecline = () => {
    setShowSeatConfirmation(false);
    showError("Please contact the driver if you haven't seated yet.");
  };
  ...
  const handleCompleteRide = async () => {
    try {
      ...
      success("Ride completed! Thank you for using WheelShare.");
      
      // Refresh ride data to show completion
      const response = await getRideStatus(rideId);
      setRideData(response.data);
      
      // Show payment modal for carpool passenger
      setTimeout(() => {
        setShowPaymentModal(true);
      }, 1000);
    } catch (err) {
  ...
  const handlePaymentMethodSelect = async (method) => {
    ...
    try {
      if (method === 'cash') {
        await makePayment(rideId, 'CASH', null);
        setPaymentCompleted(true);
        success("Cash payment recorded. Waiting for driver confirmation.");
        
        // Don't show receipt for cash until driver confirms
        ...
        // Fetch and show receipt for online payment
        const receiptResponse = await getReceipt(rideId);
        setSelectedReceipt(receiptResponse.data);
      }
    } catch (err) {
  ...
  return (
    <div className="container" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
      ...
      >
        ...
        {rideData?.driver && (
          ...
          >
            ...
            {/* Vehicle Information - Show when ride starts */}
            {rideData?.vehicle && (rideData.status === "STARTED" || rideData.status === "COMPLETED") && (
              <div style={{
                background: 'white',
                padding: '1rem',
                borderRadius: 'var(--radius-md)',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', fontSize: '0.9rem' }}>
                  <div>
                    ...
                    <div style={{ color: 'var(--text-muted)' }}>{rideData.vehicle.vehicleType}</div>
                  </div>
                  <div>
                    <span style={{ fontWeight: 600, color: 'var(--text-main)' }}>Registration:</span>
                    <div style={{ color: 'var(--text-muted)', fontWeight: 600 }}>{rideData.vehicle.vehicleNumber}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{
              ...
              {rideData.status === "ACCEPTED" && "Driver is on the way to pick you up"}
              {rideData.status === "STARTED" && "Ride in progress - Enjoy your journey!"}
              {rideData.status === "COMPLETED" && "Ride completed successfully"}
            </div>
          </motion.div>
        )}
        ...
        <div className="flex gap-md">
          {rideData?.status === "COMPLETED" || rideData?.status === "CANCELLED" ? (
            <>
              ...
              {rideData?.status === "COMPLETED" && paymentCompleted && (
                <button 
                  onClick={async () => {
                    try {
                      const receiptResponse = await getReceipt(rideId);
                      ...
                      showError("Receipt not available yet");
                    }
                  }}
      ...
      {/* Payment Method Selection Modal */}
      {showPaymentModal && !paymentCompleted && !paymentMethodSelected && rideData?.status === "COMPLETED" && (!rideData?.paymentStatus || rideData?.paymentStatus !== 'COMPLETED') && (
        <div style={{
          position: 'fixed',
          top: 0,
          left: 0,
      ...
      {carpoolRequest && (
        <CarpoolApprovalModal
          request={carpoolRequest}
          onApprove={async (requestId) => {
            try {
              await approveRequest(requestId);
              success("Passenger added to your ride!");
              setCarpoolRequest(null);
              const response = await getRideStatus(rideId);
              setRideData(response.data);
            } catch (err) {
          ...
          onClose={() => setCarpoolRequest(null)}
        />
      )}
      {showSeatConfirmation && (
        <SeatConfirmationModal
          onConfirm={handleSeatConfirm}
          onDecline={handleSeatDecline}
        />
      )}
      {showRideCompletion && (
        <RideCompletionModal
          onComplete={handleCompleteRide}
          pickup={pickup || rideData?.sourceAddress}
          drop={drop || rideData?.destinationAddress}
        />
      )}
    </div>
  );
