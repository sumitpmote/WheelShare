...
const DriverDashboard = () => {
  ...
  // =========================
  // POLL PENDING CASH CONFIRMATIONS
  // =========================
  const pollCashConfirmations = () => {
    const interval = setInterval(async () => {
      try {
        const res = await getPendingCashConfirmations();
        if (res.data && res.data.length > 0) {
          setPendingCashPayment(res.data[0]);
        }
      } catch (err) {
        console.error("Failed to fetch cash confirmations:", err);
      }
    }, 2000);
  ...
  useEffect(() => {
    ...
    // Always poll for cash confirmations when logged in as driver
    cashInterval = pollCashConfirmations();

    return () => {
      if (locationInterval) clearInterval(locationInterval);
      if (ridesInterval) clearInterval(ridesInterval);
      if (cashInterval) clearInterval(cashInterval);
    };
  }, [online]);
  ...
  const handleConfirmCash = async (rideId) => {
    try {
      await confirmCashPayment(rideId);
    ...
    <div className="container" style={{ padding: "2rem 0" }}>
      ...
      {pendingCashPayment && (
        <CashConfirmationModal
          ...
          onClose={() => setPendingCashPayment(null)}
        />
      )}
    </div>
  );
